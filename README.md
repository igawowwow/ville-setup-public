...
途中で役割（1: dev / 2: sales / 3: back-office）とGitHub情報を聞かれる。**10〜20分かかるので、途中でターミナルを閉じずに最後の「セットアップ完了」表示まで待つこと**（アプリのインストールが途中で止まったままになる）。

終わったらターミナルを閉じて開き直し、案内された `gh auth login` 等でログイン。
LINE・tl;dvはHomebrew casksが無いため手動導入（LINEはApp Storeか line.me、tl;dvはtldv.ioのChrome拡張）。
